...
contract TradingHelper is Ownable{
    ...
    address public USDC = 0xD87Ba7A50B2E7E660f678A895E4B72E7CB4CCd9C; // Goerli Testnet
    ...
    uint256 public fundsBackTax = 50; // users should pay 0.5% of original amount
    uint256 constant feeDenominator = 10000;
    uint256 public maxTradePeriod = 6 hours;
    ...
    function setmaxTradePeriod(uint256 _period) external onlyOwner {
        require(_period >= 3 hours, "impossible to be below 3hours");
        maxTradePeriod = _period;
    }
    ...
    function SwapToWETH(uint256 _inAmount) external returns (uint _outAmount){
        if(_inAmount == 0) {
            ...
            if (IERC20(USDC).allowance(msg.sender, address(uniswapV2Router)) == 0) {
                IERC20(USDC).safeApprove(address(uniswapV2Router), type(uint256).max);
            }
    ...
    function SwapWETH(uint256 _inAmount) external returns (uint _outAmount){
        if(_inAmount == 0) {
            ...
            if (IERC20(WETH).allowance(msg.sender, address(uniswapV2Router)) == 0) {
                IERC20(WETH).safeApprove(address(uniswapV2Router), type(uint256).max);
            }

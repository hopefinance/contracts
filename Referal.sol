...
contract Referal is Ownable{
    ...
    function setOperator(address _genesis) external onlyOperator {
        require(_genesis != operator, "already set");
    ...
    function getLatestReferLogs(uint256 length) public view returns(ReferLog[] memory) {
        ReferLog[] memory latestLogs;
        uint256 k = 0;
        for(uint256 i = totalReferNum-1; i >=0; i-- ){
            latestLogs[k] = referLogs[i];
            k += 1;
            if(k == length) {
                break;
            }

...
contract Treasury is ContractGuard {
    ...
    address[] public excludedFromTotalSupply = [
        address(0x8Fdb06C8eECaFa3B0A4adCeB8cCF5df31C8b96bC) // HopeGenesisPool
    ];
    ...
    function buyBonds(uint256 _hopeAmount, uint256 targetPrice) external onlyOneBlock checkCondition checkOperator {
        ...
        uint256 _bondAmount = _hopeAmount.mul(_rate).div(1e17);
    ...
    function redeemBonds(uint256 _bondAmount, uint256 targetPrice) external onlyOneBlock checkCondition checkOperator {
        ...
        uint256 _hopeAmount = _bondAmount.mul(_rate).div(1e17);
        ...
        if (epoch < bootstrapEpochs) {
            ...
            if (previousEpochHopePrice > hopePriceCeiling) {
                ...
                if (seigniorageSaved >= bondSupply.mul(bondDepletionFloorPercent).div(10000)) {
                    ...
                    _savedForBoardroom = hopeSupply.mul(_percentage).div(1e17);
                } else {
                    // have not saved enough to pay debt, mint more
                    uint256 _seigniorage = hopeSupply.mul(_percentage).div(1e17);
